...
{    // SW2 handler
    ...
    if (abs(diff - 1077325) < 77325)
    {
        timeSeries[0] = prev;
        counter = 1;
        startFlag = 1;
        buttonStartTime = Timer_IF_GetCount(g_ulRefBase, TIMER_A);
        Timer_IF_Stop(g_ulRefBase, TIMER_A);
    }

    prev = current;
    if (startFlag == 1 && diff > 50000)
    {
        timeSeries[counter] = current; //get current time
        counter++;
    }
...
void printChar(char ch, short position)
{
    int textsize = 1;
        ...
        while (1)
        {
            ...
            else
            {
                ...
                if (button != -1)
                {
                    ...
                    {
                        position = -1;
                        printChar(button + '0', position);
                        buffer[strlen(buffer) - 1] = '\0';
                    }
                    else
                    {

                        if (button == prev && interval < 30000000)
                        {
                            pressTime++;
                            position = 0;
                        }
...
//               oledTest(); // print to screen;

                ulStatus = MAP_GPIOIntStatus(sig_input.port, false);
                MAP_GPIOIntClear(sig_input.port, ulStatus);
                MAP_GPIOIntEnable(sig_input.port, sig_input.pin);
                Timer_IF_Start(g_ulRefBase, TIMER_A, 1000);
                buttonEndTime = Timer_IF_GetCount(g_ulRefBase, TIMER_A);
